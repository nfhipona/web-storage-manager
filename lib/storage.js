...
class WebStorage {
   ...
   */
  setItem(key, value) {
    let encoded = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    try {
  ...
  appendItem(key, value) {
    try {
      const data = _classPrivateFieldGet(this, _storage).getItem(key);
  ...
  updateItemInItem(keyPath, value, attrCompare) {
    try {
      ...
      const data = _classPrivateFieldGet(this, _storage).getItem(parentKey);
      const r = this.isDataEncoded(data);
